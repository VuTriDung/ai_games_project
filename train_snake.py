...
class SnakeEnv:
    ...
    def get_state(self):
        ...
        dir_l, dir_r, dir_u, dir_d = self.dir == LEFT, self.dir == RIGHT, self.dir == UP, self.dir == DOWN
        
        danger_s = (dir_r and self.is_collision(point_r)) or (dir_l and self.is_collision(point_l)) or (dir_u and self.is_collision(point_u)) or (dir_d and self.is_collision(point_d))
        danger_r = (dir_u and self.is_collision(point_r)) or (dir_d and self.is_collision(point_l)) or (dir_l and self.is_collision(point_u)) or (dir_r and self.is_collision(point_d))
        danger_l = (dir_d and self.is_collision(point_r)) or (dir_u and self.is_collision(point_l)) or (dir_r and self.is_collision(point_u)) or (dir_l and self.is_collision(point_d))
    ...
    def is_collision(self, pt):
        return pt[0] < 0 or pt[0] >= self.w or pt[1] < 0 or pt[1] >= self.h or pt in self.snake[1:]
    ...
    def step(self, action):
        ...
        if action == 1:   self.dir = clock_wise[(idx + 1) % 4]
        elif action == 2: self.dir = clock_wise[(idx - 1) % 4]
...
def train():
    ...
    for i in range(episodes_trained, target_episodes):
        state = env.reset()
        if state not in q_table: q_table[state] = [0, 0, 0]
        ...
        while True:
            ...
            if new_state not in q_table: q_table[new_state] = [0, 0, 0]
        ...
        # In log mỗi 5000 ván
        if (i+1) % 5000 == 0:
            print(f" -> Đã chơi {i+1:,} ván | Epsilon: {epsilon:.3f} | Kỷ lục lô này: {max_score_in_batch} điểm")
            max_score_in_batch = 0 
