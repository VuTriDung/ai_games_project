...
async function saveFlappyWeights(
  weights: number[],
  generation: number,
  bestScore: number,
  allTimeBest: number,
...
class NeuralNet {
  ...
  mutate(rate: number): NeuralNet {
    return new NeuralNet(
      this.w.map((x) => (Math.random() < rate ? x + randn() * 0.5 : x)),
    );
  }

  clone(): NeuralNet {
    return new NeuralNet(this.w);
  }
}
...
class Bird {
  ...
  update(pipes: Pipe[]): void {
    ...
    if (this.y - BIRD_R < 0 || this.y + BIRD_R > H) {
      this.alive = false;
      return;
    }
    ...
    if (near) {
      ...
      if (
        this.net.forward((this.y - gc) / H, nearDist / W, this.vy / 10, gc / H)
      ) {
        this.vy = JUMP;
      }
  ...
  draw(ctx: CanvasRenderingContext2D): void {
    ...
    ctx.beginPath();
    ctx.ellipse(-4, wf, 9, 5, -0.4, 0, Math.PI * 2);
    ctx.fill();
...
function drawBackground(ctx: CanvasRenderingContext2D): void {
  ...
  for (const [cx, cy, rw, rh] of clouds) {
    ...
    ctx.fill();
    ctx.beginPath();
    ctx.ellipse(cx + 24, cy - 10, rw * 0.6, rh * 0.8, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.beginPath();
    ctx.ellipse(cx - 20, cy - 6, rw * 0.5, rh * 0.7, 0, 0, Math.PI * 2);
    ctx.fill();
  }
...
export function startFlappyGA(
  ...
  function doEvolve(): void {
    ...
    const avg = Math.round(
      birds.reduce((s, b) => s + b.fitness, 0) / birds.length,
    );
  ...
  function tick(): void {
    ...
    if (pipes.length < 4) {
      ...
      if (lastX < W + 200)
        pipes.push(new Pipe(lastX + 250 + Math.random() * 80));
    }
  ...
  function loop(): void {
    ...
    ctx.fillText(
      `Gen ${generation}  |  Alive: ${alive}/${cfg.population}  |  Score: ${bestScore}  |  Best: ${allTimeBest}`,
      12,
      24,
    );
...
document
  .getElementById("btn-exit-flappy")
