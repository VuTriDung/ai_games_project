...
class Game2048 {
  ...
  move(direction: "up" | "down" | "left" | "right"): boolean {
    ...
    moved = !this.grid.every((row, r) =>
      row.every((v, c) => v === oldGrid[r][c]),
    );
...
function drawGrid() {
  ...
  if (game.isOver) {
    ...
    statusSpan.textContent =
      "🎉 Bạn đã đạt 2048! Tiếp tục chơi để ghi điểm cao hơn.";
  } else {
...
async function getAIMove(grid: number[][]): Promise<string | null> {
  try {
    ...
    statusSpan.textContent =
      "⚠️ Không kết nối được server AI. Vui lòng chạy server.py.";
    return null;
  }
}
