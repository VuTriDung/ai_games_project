...
document.getElementById("btn-start-connect4")?.addEventListener("click", () => {
  ...
  document.getElementById("connect4-status")!.innerText =
    "Human vs Minimax (Yellow)";
});
document
  .getElementById("btn-start-connect4-mcts")
...
function showResult(winner: number | null) {
  if (aiPlayer === null) {
    ...
    realStats.connect4.r_games++;
    realStats.connect4.y_games++;
...
async function aiMove() {
  ...
  try {
    ...
    document.getElementById("connect4-status")!.innerText = "Lỗi kết nối API";
  }
}
